...
Widgets: column,row (props: spacing,align) · group (title) · scroll · separator · spacer (size) · label (text,size:"small|body|heading|title",color:"#rrggbb",bold) · button (text,on_click) · text_input (bind,placeholder,multiline) · slider (bind,min,max,step,on_change) · checkbox/toggle (bind,text,on_change) · dropdown (bind,options:["a","b"] or "${listExpr}",on_change) · list_view (items:"${listExpr}",template:"${i+1}. ${item}",on_remove — handler sees locals item,i) · canvas (width,height,draw:"${listExpr}" of maps {kind:'circle'|'rect'|'line'|'text',x,y,r|w,h|x2,y2|text,size,color:'#hex'},on_pointer — locals px,py) · progress (value 0..1) · if (cond, then-children in "children").

An <action> is {"do":"<verb>",...}: set{target,value} · toggle{target} · inc{target,by} · push{target,value} · remove_at{target,index} · clear{target} · set_key{target,key,value} (map states) · if{cond,then:[...],else:[...]} · emit{handler} (no cycles) · notify{title,body} · window{op:"close"|"set_title",title} · timer{op:"start"|"stop"|"reset",id}.
