...
| G2 | **Pinch** 👌 | thumb–index fingertips touch | Primary select / click; **pinch-hold + move = drag — including 3D objects: pinching a stage object grabs it (spring), release throws** *(2026-08-02: object manipulation moved from ✊ to 👌 for mouse-click parity)* | Highest-precision fingertip event a webcam can see; maps to "pick precisely". Enter < 25 mm, exit > 40 mm (hysteresis). |
...
| G4 | **Grab** ✊ | all fingers curled to fist | **The space and windows**: over a window = drag it; over the stage = orbit the camera. *(2026-08-02: objects moved to 👌 pinch — ✊ was overloaded and colliding with 👍.)* Classifier note: 👍/👎 now require a long, clearly vertical thumb — a fist's wrapped thumb always reads Grab. | Whole-hand closure for whole-scene manipulation; the fist steers the world. |
