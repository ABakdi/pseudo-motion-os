...
- [~] **3. CSL sign engine + RECORD** — engine + RECORD shipped (COMMAND/CANCEL pending): — motion-primitive layer over the pose stream (HOLD/SQUEEZE/FLICK_OUT/PUSH FSMs, Midas guards, refractory); first sign: ✋→✊ squeeze toggles voice capture. *(Spec: [[Computer Sign Language#3]]–[[Computer Sign Language#4|4]])*
