...
- [x] **1. Browser: actually loads and is interactive** — root cause found by live-debugging the deployed build in Chrome: `windows()` computed the browser view rect into a **local variable and never wrote it back to `self.browser_view`**, so the platform iframe never received a URL — broken for every site since M8. Fixed (one assignment); embed-refusal quick links were verified against real response headers earlier (Wikipedia/HN send no frame-blocking headers). Interaction inside the page = native iframe input (nothing to build). *(Verified live: Wikipedia renders and scrolls inside the Browser window.)*
- [x] **2. Remove the Launcher** — overlay, palm-hold trigger, dock ◆ entry, and `launcher` palette command all deleted (user decision); dock + palette are the launching surfaces. The palm now belongs to RECORD.
- [x] **3. CSL sign engine + RECORD + COMMAND** — engine, RECORD (still ✋ hold) and COMMAND (still ☝ hold, chin anchor waits for the face mesh) shipped; CANCEL (palm push) deferred — needs palm-z velocity: — motion-primitive layer over the pose stream (HOLD/SQUEEZE/FLICK_OUT/PUSH FSMs, Midas guards, refractory); first sign: ✋→✊ squeeze toggles voice capture. *(Spec: [[Computer Sign Language#3]]–[[Computer Sign Language#4|4]])*
- [x] **4. Voice Kit widget + continuous transcription** — top-right chip (● REC pulsing, never hidden while live) with expandable live transcript; the Whisper engine is now continuous (utterance end flushes the buffer and keeps rolling); capture auto-starts at boot when the mic was granted; RECORD ✋-hold and the chip click toggle it.
- [x] **5. Voice persistence + search + →note** — `/voice/<date>/s<id>.json` rewritten per utterance (crash-safe); widget search scans all sessions; →note converts a session to markdown (commands as a checklist) in /notes. *(Terminal `voice <query>` sugar deferred — `cat` works.)*
- [x] **6. COMMAND sign + command stream** — ☝ held still 0.8 s while capturing arms ⌘ (toast + chip badge); the next utterance renders accent-⌘ in the kit, persists flagged, and routes through the palette brain. *(FLICK_OUT + chin anchor upgrade lands with face-anchored signs.)*
- [x] **7. AI context envelope** — voice commands to the assistant carry `[context] stage · open windows [/context]`. *(Focused-object + recent-transcript fields grow with pinch-select focus.)*
- [x] **8. AI web access** — `web_open` (drives the visible Browser), `web_search` (Wikipedia OpenSearch), `web_fetch` (Jina Reader) — async through the new `WebFetch` syscall / `WebResult` event (ABI 1.11); the tool loop suspends and resumes when the body arrives.
- [~] **9. Two-handed 3D grammar** — **two-✋-palm spread/approach zooms the camera** (shipped); pinch already grabs/moves/throws objects. Deferred: persistent pinch-select focus + non-dominant rotate/scale of the focused object.
...
- [~] **Face gestures (opt-in)** — **double-blink = click at the cursor** shipped (kernel blink FSM over eyeBlink blendshapes, 0.7 s window, click injected platform-side). Brow-raise confirm + mouth-"O" push-to-talk pending.
