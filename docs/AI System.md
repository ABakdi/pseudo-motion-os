...
- v1 tools: `sys_query` · `fs_list` · `fs_read` (truncated at 4 KB) · `fs_write` · `app_open`, plus the **stage tools** (2026-08-01): `stage_spawn` / `stage_list` / `stage_remove` / `stage_clear` / `stage_push` (physics impulse — animation via the physics engine) / `stage_light` (sun azimuth/elevation/intensity/ambient). The budget is 8 calls per request so the assistant can compose little "models" from several primitives ("build me a tower", "make it sunset", "throw the red cube"). All read/reversible-write (Tier 0–1): reads are silent, every call shows a `🔧 tool args` line in the palette, and writes additionally raise a toast — **nothing acts invisibly**. Tier-2 arrived 2026-08-02: `fs_delete` runs only through the consent sheet (modal card, 👍/👎/brow-raise/Esc; denial is final and reported to the model).
